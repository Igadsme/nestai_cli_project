...
def make_json_safe(obj: Any) -> Any:
    ...
    Handles:
      - Custom classes exposing .to_dict()
      - Dataclasses
      - dicts / lists / tuples
      - Primitive types
      - Falls back to repr() for anything else
    """
    if obj is None:
        return None

    # Custom class with to_dict()
    if hasattr(obj, "to_dict") and callable(getattr(obj, "to_dict")):
        try:
            return make_json_safe(obj.to_dict())
        except Exception:
            # If to_dict fails, fall through to other branches
            pass
...
def get_console(theme: Optional[str]) -> Console:
    """Create a Rich console with optional light/dark theme hint."""
    if theme == "light":
...
def gradient_title(text: str) -> Text:
    """Simple gradient-style title text."""
...
def _safe_agent_dict(agent: Any) -> Dict[str, Any]:
    """Normalize any agent object/dict into a safe dict."""
    return make_json_safe(agent) or {}
...
# ================================================================
# SECTION RENDERERS – ALWAYS AUTO-EXPAND
# ================================================================
...
def print_attack_sim_section(console: Console, attack_result: Any) -> None:
    ...
    header = Text("Attack Simulation", style="bold white")
    header.append("  ")
    header.append(risk_badge(risk))
    ...
    if weak_points:
        ...
        console.print("  [dim]No weak points reported.[/dim]")
    console.print()
...
def handle_code_preview(console: Console, code: str, path: Path, minimal: bool = False) -> None:
    ...
    answer = Prompt.ask(
        "Press [bold]<Enter>[/bold] to preview the first ~40 lines, or type [bold]s[/bold] to skip",
        default="",
        show_default=False,
    ).strip().lower()

    if answer in {"s", "skip", "no", "n"}:
        console.print("Code preview skipped.")
        return
...
def generate_html_report_from_last() -> Optional[Path]:
    ...
    html.append(
        ...
        "background:#050816;color:#f8fafc;padding:20px;}"
        "h1,h2{color:#38bdf8;}"
        "pre{background:#020617;padding:12px;border-radius:8px;color:#e5e7eb;}"
        "</style>"
    )
    html.append("</head><body>")
    html.append("<h1>NESTAI – Secure Coding Report</h1>")
    html.append(f"<h2>Original Prompt</h2><pre>{data.get('original_prompt','')}</pre>")
...
def run_report_command(args: List[str], console: Console) -> None:
    ...
    if not path:
        console.print("[red]No last result found. Run a NestAI scan first.[/red]")
        return
...
def run_pipeline_with_ui(
    ...
    user_prompt: str,
    minimal: bool = False,
    verbose: bool = False,  # kept for signature compatibility
) -> None:
    _ensure_dirs()
    ...
    with console.status("[cyan]Running NestAI secure pipeline…[/cyan]", spinner="dots"):
        start = time.time()
        raw_result: Any = run_nestai(user_prompt)

        # In case some version returns (result, ) or (result, meta)
    ...
    # Save last result for HTML/dashboard usage
    save_last_result(result)

    # Build pseudo stage timings
    stages = [
        "Static Analysis",
        "Red Team",
        "Malicious Gate",
    ...
    # Optional finishing progress bar
    if not minimal:
        progress = Progress(
    ...
    # AUTO-EXPAND ALL FINDINGS (no prompts)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        "--minimal",
        action="store_true",
        help="Minimal summary-only output (no progress animation).",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="(Reserved) – kept for compatibility; output is already detailed.",
    )
    parser.add_argument(
        "--theme",
        choices=["dark", "light"],
        default=None,
        help="Choose CLI theme colors.",
    )
    parser.add_argument(
        "command",
        nargs="+",
        help="Prompt or subcommand (history, config, report).",
    )

    args = parser.parse_args(argv)
    cmd_parts: List[str] = args.command
    ...
    # Otherwise treat the entire command as a user prompt
